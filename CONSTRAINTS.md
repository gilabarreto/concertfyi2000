...
| CSP sem `frame-ancestors`, `report-uri` nem `sandbox` | A política vai num `<meta http-equiv>` porque o GitHub Pages não deixa mandar cabeçalho de resposta, e a especificação manda o navegador **ignorar** essas três diretivas quando vêm por `<meta>`. Na prática: não há defesa de clickjacking e nenhuma violação é reportada — só aparece no console de quem estiver olhando. O resto da política vale igual. As três só voltam saindo do GitHub Pages, que já está registrado como decisão do dono. | Victor | junto com a decisão sobre hospedagem |
